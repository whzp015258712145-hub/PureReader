...
- **Base Language First Development Workflow (基准语言优先开发模式):** Default fallback languages are English (`en`) and Simplified Chinese (`zh-Hans`). During active feature iteration, implement and verify UI/features using `en` and `zh-Hans` first. Do NOT spend cycles translating to all 14 languages for WIP features. Expand translations to all 14 languages in batch prior to release/stabilization milestones.
